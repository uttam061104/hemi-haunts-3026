  ...
  return (
    <group>
      ...
      <mesh rotation-x={-Math.PI / 2} position={[0, 0.02, -10]}>
        ...
        <meshStandardMaterial color="#1d1f24" roughness={1} />
      </mesh>

      {/* graveyard soil */}
      <mesh rotation-x={-Math.PI / 2} position={[0, 0.03, -54]}>
        <planeGeometry args={[64, 32]} />
        <meshStandardMaterial color="#191b16" roughness={1} />
      </mesh>
        ...
        <group key={`b${i}`} position={[b.x, 0, b.z]}>
          <mesh position-y={b.h / 2} castShadow receiveShadow>
            ...
            <meshStandardMaterial
              color={`rgb(${Math.round(26 + b.tone * 26)}, ${Math.round(24 + b.tone * 22)}, ${Math.round(30 + b.tone * 26)})`}
              roughness={0.95}
              flatShading
            />
          </mesh>
          ...
          <mesh position-y={b.h + 0.7} rotation-y={Math.PI / 4}>
            ...
            <meshStandardMaterial color="#0f1013" flatShading roughness={1} />
          </mesh>
          {/* a lit window or two */}
          {i % 3 === 0 && (
      ...
      {data.fences.map((f, i) => (
        <mesh key={`f${i}`} position={[f.x, 0.9, f.z]} rotation-y={f.rot}>
          ...
          <meshStandardMaterial color="#101116" flatShading roughness={1} />
        </mesh>
      ))}
      ...
      {GRAVES.map((g, i) => (
        <group key={g.name} position={[g.x, 0, g.z]}>
          <mesh position-y={0.15}>
            ...
            <meshStandardMaterial color="#16181a" flatShading roughness={1} />
          </mesh>
